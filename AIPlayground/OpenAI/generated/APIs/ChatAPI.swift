...
protocol ChatAPITransport: Sendable {
    func execute(request: Request) async throws -> (Data, URLResponse)
}

protocol ChatAPIClientMiddleware: Sendable {
    func intercept(
        _ request: Request,
...
open class ChatAPI: ChatAPIEndpoints, @unchecked Sendable {
    internal var transport: ChatAPITransport
    internal var middlewares: [ChatAPIClientMiddleware]
    internal var basePath: String
    internal var jsonDecoder: JSONDecoder

    init(basePath: String, transport: ChatAPITransport, middlewares: [ChatAPIClientMiddleware], jsonDecoder: JSONDecoder = JSONDecoder.default) {
    ...
    open func createChatCompletion(createChatCompletionRequest: CreateChatCompletionRequest) async throws -> CreateChatCompletionResponse {
        ...
        return try await send(request: urlRequest) {
            try self.jsonDecoder.decode(CreateChatCompletionResponse.self, from: $0)
        }
    }
    /**
     Creates a model response for the given chat conversation.
...
protocol ChatAPIEndpoints {


        func createChatCompletion(createChatCompletionRequest: CreateChatCompletionRequest) async throws -> CreateChatCompletionResponse
